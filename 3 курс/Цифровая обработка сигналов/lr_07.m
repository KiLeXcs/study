...
subplot(3,1,1),stem(n,x8,'Linewidth',2),xlabel('n'), grid
...
m = -(N-1):(N-1);                % ÂÅÊÒÎĞ ÄÈÑÊĞÅÒÍÛÕ ÑÄÂÈCÎÂ ÄËß ÀÊÔ 
subplot(3,1,2),stem(n,u0_m,'Linewidth',2), grid
subplot(3,1,3),stem(m, R,'Linewidth',2),xlabel('m')	, grid
title('ACF R(m)')
disp('%')
disp('%')
disp('% Äëÿ âóâîäà ÄÈÑÏÅĞÑÈÈ àääèòèâåîé ñìåñè ñèãíàëà q øóìÿì è ÀÊÔ R(O) íàæìèòå <ENÔER>') 
pause 
disp('%')
disp('%')
...
disp('%')
disp('%')
disp('% Äëÿ Ïğîäîëæåíìÿ íàçìèòå <ENTER>')
pause 
disp('%')
disp('%')
disp('% ï.15. ÍÎĞÌÀËÜÓÉ ÁÅËÛÉ ØÓÌ Ñ ÇÀÄÀÍÅÛÌÈ ÑÒÀÒÈÑÒÈ×ÅÑÊÈÌÈ ÕÀĞÀÊÒÅĞÈÑÒÈÊÀÌÈ')
r_normMean = randn(1,10000)+Mean;      % ÍÎĞÌÀËÜÍÛÉ ÁÅËÛÉ ØÓÌ Ñ ÇÀÄÀÍÍÛÌ ÌÀÒÅÜÀÒÈ×ÅÑÊÈÌ ÎÆÈÄÀÍÈÅÌ
r_normVar = sqrt(Var).*randn(1,10004); % ÍÎĞÌÀËÜÍÛÉ ÁÅËÛÉ ØÓÌ Ñ ÇÀÄÀÍÍÎÉ ÄÈÑÏÅÑÑÈÅÉ
r_normMeanVar = sqrt(Var).*randn(1,10000)+ Mean; % ÍÎĞÌÀËÜÍÛÉ ÁÅËÛÉ ØÓÌ`Ñ ÇÀÄÀÍÍÛÌÈ ÊÀÒÅÌÀÒÈ×ÅÁÊÈÌ ÎÆÈÄÀÍÀÅÌ H ÄÌÑÏÅĞÑÈÅÉ
MAX = max([r_normMean r_normVar r_normMeanVar]); 
% ÈÀÊÑÈÌÀËÜÍÎÅ ÇÍÀ×ÅÍÈÅ ØÓÌÀ ÑĞÅÄÈ ×ÅÒÛĞÅÕ ÅÃÎ ĞÀÇÍÎÂÈÄÅÎÑÒÅÉ
disp('%')
disp('%')
disp('% Äëÿ âûâîäà ÃĞÀÔÈÊÆÂ íîğìàëüíîãî áåëîãî øóìà íàæìèòå <ENTER>')
pause
