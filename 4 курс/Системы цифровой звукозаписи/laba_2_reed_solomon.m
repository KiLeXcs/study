...
f_150_and_4900_and_11000_192000 = sin(2 * pi * 150 * t_192000) + sin(2 * pi * 4900 * t_192000) + sin(2 * pi * 11000 * t_192000);
...
function sampling = sample(x, r, t, s_0, name, f_signal)
   ...
   x = reshape(x, 1, []); %from mono to stereo
   num_blocks = length(x) / r;
   y = [];
   for i = 0:num_blocks-1
       idx_start = i * r + 1;
       idx_end = (i + 1) * r;
       block = x(idx_start:idx_end);
       y = [y, block, block];
   end

   %--------------------------------------------coder--------------------------------------------

   pol = rsgenpoly(255, 251, 285);

   enc = comm.RSEncoder(255, 251, pol, 24, 'BitInput', true);
   input_length = length(y);
   block_size = 192;
   num_blocks = ceil(input_length / block_size);
   enc_m = [];

   for i = 1:num_blocks
       start_idx = (i-1)*block_size + 1;
       end_idx = min(i*block_size, input_length);
       current_block = logical(y(start_idx:end_idx)).'; 

       if length(current_block) < block_size
           current_block = [current_block; false(block_size - length(current_block), 1)];  % ← ошибка!
       end
       enc_block = enc(current_block(:));
       enc_m = [enc_m; enc_block];
   end
   ...
   dec = comm.RSDecoder(255, 251, pol, 24, 'BitInput', true);
   ...
   for i = 1:num_blocks
       start_idx = (i-1)*block_size + 1;
       end_idx = min(i*block_size, input_length);
       current_block = y(start_idx:end_idx).';
       
                    
        if length(current_block) < block_size
            current_block = [current_block; false(block_size - length(current_block), 1)];
        end
        decoded_block = dec(current_block(:));
        dec_m = [dec_m; decoded_block];
   end

   %----------------------------------digital-to-analog converter--------------------------------
   
   y = dec_m;
   num_blocks = length(y) / (2*r); % from stereo to mono
   x1 = [];
   for i = 0:num_blocks - 1
       start = i * 2*r + 1;
       first_half = y(start : start + r - 1);
       x1 = [x1, first_half];
   end
   x = reshape(x1, r, []);
   ...
   if length(z) > length(t)
       z = z(1:length(t));
   elseif length(z) < length(t)
       z(end+1:length(t)) = z(end);
   end
